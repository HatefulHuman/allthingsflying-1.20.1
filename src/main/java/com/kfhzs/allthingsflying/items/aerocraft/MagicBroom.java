...
public class MagicBroom extends Aerocraft {
    public MagicBroom() {
        ...
        consumer.accept(new IClientItemExtensions() {
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                if (this.renderer == null) {
                    this.renderer = new AerocraftRender("magic_broom");
                }
                return renderer;
            }
        });
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.getMainHandItem().is(this)) {
            if (EngineHelper.hasEngine(player.getMainHandItem())) {
                MagicBroomEntity boat = new MagicBroomEntity(level, player.getX(), player.getY(), player.getZ(), player.getMainHandItem());
