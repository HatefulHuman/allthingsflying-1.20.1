...
public class MagicBroomEntity extends AerocrafEntity {
    private float friction;
    private float stopThreshold;
    private float acceleration;
    private static final String[] ACTIONS = {"zuoxia", "zuoxia2", "qixing", "tangxia", "zhanli", "zhanli2", "zhanli3"};

    public MagicBroomEntity(EntityType<?> aSuper, Level level) {
        super(aSuper, level);
        // 骑乘时取消重力
    ...
    public MagicBroomEntity(Level level, double p_38294_, double p_38295_, double p_38296_, ItemStack stack) {
        this(EntityRegister.MAGIC_BROOM.get(), level);
    ...
    public void tick() {
        ...
        if (EngineHelper.getEngineItem(this.getItemStack()).is(ItemsRegister.THERMAL_ENGINE.get())) {
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnThermalEngineParticles(this, 0.25,1.5,0.5);
            } else {
                spawnThermalEngineParticles(this, 0.15,1.5,0.5);
            }
        }else if (EngineHelper.getEngineItem(this.getItemStack()).is(ItemsRegister.CLOUD_ENGINE.get())) {
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnCloudEngineParticles(this, 0.15,1.5,0.5);
            }
            ...
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnWhiteLightParticles(this, 1.0,1.5,0.5);
            }
        }
    ...
    public double getPassengersRidingOffset() {
        ...
        if (this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[0])
                ...
                || this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[3])) {
            return 0.3f;
        } else if (this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[2])) {
            return 0.0f;
        } else {
            return 1.0f;
        }
    ...
    protected void positionRider(Entity entity, Entity.MoveFunction moveFunction) {
        if (this.hasPassenger(entity)) {
            ...
            float f1 = (float) ((this.isRemoved() ? 0.009999999776482582 : this.getPassengersRidingOffset()) + entity.getMyRidingOffset());
            if (this.getPassengers().size() > 1) {
                ...
                if (i == 0) {
                    ...
                    if (this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[0])
                            || this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[1])
                            || this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[2])
                            || this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[3])) {
                        f = -0.6F;
                        f1 += 0.0F;
                    }else {
                ...
                if (entity instanceof Animal) {
                    f += this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[3]) ? -0.2f : 0.2F;
                }
            }
            ...
            entity.setYRot(entity.getYRot() + (this.getYRot() - this.yRotO));
            entity.setYHeadRot(entity.getYHeadRot() + (this.getYRot() - this.yRotO));
