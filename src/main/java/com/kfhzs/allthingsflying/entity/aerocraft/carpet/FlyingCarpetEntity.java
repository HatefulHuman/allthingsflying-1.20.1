...
public class FlyingCarpetEntity extends AerocrafEntity {
    ...
    public FlyingCarpetEntity(EntityType<?> aSuper, Level level) {
        super(aSuper, level);
        // 骑乘时取消重力
    ...
    public FlyingCarpetEntity(Level level, double p_38294_, double p_38295_, double p_38296_, ItemStack stack) {
        this(EntityRegister.FLYING_CARPET.get(), level);
    ...
    public void tick() {
        ...
        if (EngineHelper.getEngineItem(this.getItemStack()).is(ItemsRegister.THERMAL_ENGINE.get())) {
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnThermalEngineParticles(this, 0.25, 3.25, 0.5);
            } else {
                spawnThermalEngineParticles(this, 0.15, 3.25, 0.5);
            }
        } else if (EngineHelper.getEngineItem(this.getItemStack()).is(ItemsRegister.CLOUD_ENGINE.get())) {
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnCloudEngineParticles(this, 0.15, 3.25, 0.5);
            }
            ...
            if (this.playerSpeed > 0.2 && !this.isInWater()) {
                spawnWhiteLightParticles(this, 1.0,3.25,0.5);
            }
        }
        ...
        if (this.hasPassenger(entity)) {
            ...
            if (this.getPassengers().size() > 1) {
                ...
                if (entity instanceof Animal) {
                    f += this.entityData.get(DATA_ID_PLAYER_ACTION).equals(ACTIONS[2]) ? -0.2f : 0.2F;
                }
            }
